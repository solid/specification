...
* WT: Based on a few people who raised the obsoleted HTTP specifications in our reports, I created a few PRs that make necessary changes to used the new ones. All but the Protocol are extremely straightforward. Only in the Solid Protocol there's a need for more reviews. Double check that I didn't miss anything that would affect the conformance. 
