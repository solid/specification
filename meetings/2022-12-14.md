...
* SC: You are cutting out a bit. Those who have high level view on user stories, some people who tag it initially, but people need to have awareness of all the user stories so they can manage it that way.  Maybe this is something we can follow up on. The labels, most do not have a description, but we could use better descriptions. Some are clear enough. 
* TBL: We also have a roadmap we could tie this to, things like media.
* WT: Thanks Sarven and Tim. Useful to have a more clear understanding of how or if the structure of the user stories, so we don't expect everyone to go through them all, but some indication this might be useful to me cause I'm working on X or Y.
* SC: The readme hints at a general form, As a User ... the form
...
* TBL: The person originally brought up the importance of use cases, are they on this call?
...
* SC: Is the survey collecting what we need? We could put together survey to collect information about where people are finding info, also how people are being notified about meetings etc.  I could create an issue on that and we can come up with survey questions and look at it in follow-up meetings. Any ideas on that?  The scheduling, whether meeting time is ok, or languages that user stories are available in, I'm just looking through the survey and thinking of how it applies to use cases.
...
* SC: In context we are talking bout, which use cases may be of interest to implementors.
* SC: Let me pull this link up, I've been pushing this for a while, but then we put it aside. 
* SC: https://github.com/solid/authorization-panel/blob/main/proposals/authorization-ucr/uc-survey.md is one survey to collect some data about proposed use cases, commitment to implement or already implementing. This is quite important before dividing into actual requirements in a spec. It is also good signal to the community whether a feature will actually be implemented - towards "adequate implementation experience" as per W3C Process.
...
* SC: Use case being proposed, anyone contributing can say yes it is worth doing or I'm kind of for it and can live without it, and other options. This is the template I was proposing our understanding of the use cases and how it maps to a requirement in a spec. 
...
* SC: This is one of the steps just before writing a line of the spec. Just because someone proposed a use case, doesn't mean there has to be a requirement from it. It's to get a signal from the community that there is a significant amount of parties interested in a requirement for this use case. 
* PA: I see in Issue #9 one issue document per spec, I'm not sure this is the right granularity. For example, for SPARQL, specifications are focused on a particular use case might be transversal, might gut feeling is it might be more efficient to have use cases pointing to the specification they touch on, not necessarily to segregate use cases per specification.
* WT: +1
...
* SC: We could. Accidental bottom-up that we have general need for a component like authorizations or notifications, some spec is needed focused only on that. You can take a requirement like creating a container vice listening to updates to a container, they could go in same use case document, the approach here was that they are specific enough so you could have different solutions, so notifications could be a solution to listening and leave that in its own document. Since this is somewhat bottom-up, we could merge or create a document that touches these different areas. 
* WT: Don't stop on account of me. I wanted to say to give the example about notifications, a use case involving notifications is ever completed without also some implementation of authentication and authorization. So people from authorization have to know to look there as well that involve authorization but are not in our use cases, so if in one document, easier to find.
* SC: Good clarification. Thank you for raising that. We should have a closer look at how we make it more obvious and we could have See Also's, but I suppose in case of authorization, you would need use cases around notifications or considerations of how spec would need to advise while direct access may not be enabled through the spec, information can be shared in other ways, implementors may need to be aware. A spec could expose info not intended by authorization spec. Something we need to discuss in panels as well. 
