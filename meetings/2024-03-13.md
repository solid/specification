...
* eP: We can't put all discussions into done because sometimes we decide to come back to discuss. After the meeting we know we are not "done" and need to come back at osme point. Perhaps a way to mark as in progress?
...
* SC: Low priority. Goes along with ethical web and Solid vision, etc. Considerations on web being for everyone, and so on. Sustainability is one of those things; lots of interest in W3C around this topic. Taking something on in the group would require a lot of thought, but helps make sure what we are working on ???. I suggest people get a feel for what these documents include. 
...
* VB: Proposed by eP.
* eP: Created issue 636 to track some of the related issues. The original issue 514 which was created a year ago. In the whole year we didn't process, document, capture. Sounds reasonable to have a new work item. As long as we have a clear place that people can find re potential security vulnerabilities. In some ways addressing some of the problems. Doesn't have to be perfect but better way than addressing security related issues.
* VB: Work item sounds good.
* SC: I'm for the idea to more visibility. Suggest to have more care on the language, best practice or guideline? If BP make sure there's sufficient data that signals that. Fine as a work item (best practice and guidelines).
...
* SC: Title is not the issue but the language that's used in the document. Can discussed later.
* RG: Let's make a work item and wait for it to reach a certain maturity to add to TR.
* eP: As a rule of thumb: possibly threats and counter measures. If not conflicting with the spec, it is fair / valid approach. At least it is conforming for Solid group of specifications. If measure is passing the Solid test suites, it'd be useful to share it with others.
...
* eP: Where should it go? It'd good to merge somewhere and then keep iterating. I'm the only one listed at the moment. I want to ask others to collaborate.
...
* SC: Hard to say what best place is. Keep it in its own repo for now. Security practices cross all specs not just Protocol. If we want to always work on it we can think of it as a live document. I don't see it the same as the Solid Protocol
...
* VB: We already have w3c-cg/solid CONTRIBUTING.md for Contributing for all sorts of stuff. Ther is a section on meeting minutes recently. This PR is specifically about the Contributing Guide itself. I've approved it. It has been open for about a week? If you want to make a suggestion to the Contributing, this sets some expectations on processing.
* eP: Right, if no objections, lets merge. I've approved.
* VB: Merged.
* SC: Minor comment: we have contributing guide because it's a rough approximation of what seems to work in the group. We do't always hit deadlines or follow to the dot, but the more wwe take it seriously we speed things up and there's less chatter on how to work. Not about watching over each other's shoulder. 
