...
* HZ: are there were plans for other open source projects. 
* RG: cannot navigate CSS and need someone from CSS to help - Pavlik may be able to support here.
* JZ: Michiel and others are involved in a project called Pivot and intention is to make a fork of CSS and use that as basis as solidcommunity.net etc...possible route.
...
* HZ: Inrupt has not made a decision yet and hope to have one end of week. 
...
* JZ: Yes, agree that working on a way to present a view. Disagree that this is bound to an application. Purpose is to have a shape that describes the data so any organisation can use the shape to present the data. Goal to have client-to-client of shapes that are interoperable. I am working on one way of presenting a view of those shapes. Anybody else could create a different view.
...
* JZ: Suggested resources should be made available but was put down to say CG was about forming specifications and not for us to outreach to communities, publicise things, drawing wider community. This was when CG was reorganising, and so practitioners was formed as part of solid team - that's it's purpose to grow forum, solidproject.org, github things that are managed etc. 
* HZ: For the record, when I volunteered for co-chair I said my role would be to outreach and grow the community. So do not agree that the CG is about that - I feel it is exactly that.
