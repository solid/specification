...
* WT: Proposal is based on stuff already in SAI, but tries to take out a specific part that doesn't belong with the rest, and make it a separate specification. It is concerned with agents/client who want to discover info about some entity; they can not only find public info available in identity document, but can also find an easy way to know where the info specific to that client lives. 
* WT: One approach would be to have a lot of `seeAlso`s in the profile document and have the client check all of them. ??? proposes to have a microservice/hub to which an entity within their profile doc points, and the client can ask the service for a pointer to the place where their specific data can be found. 
