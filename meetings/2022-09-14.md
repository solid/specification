...
* TimBL: The Solid protocol, the way clients talk to servers, is described by the [Solid ecosystem roadmap](https://solidproject.solidcommunity.net/Specification/Roadmap/index.ttl#this). There are two roadmaps. One for SolidOS; one for the ecosystem. They're ordered by priority. There are some protocol oriented things on here. We don't have the status of working on things like "contacts," "calendars," "photos," and "music." So, we need another task manager to manage the verticals. A huge amount of the value comes from those client specs, so we need a roadmap for those too. The Solid protocol shouldn't change too often. The important thing is to build really cool apps that work together.
...
* TimBL: Languages we certainly do at the moment. If my Pod doesn't have the label to do something, they can go back. People will realize that it's useful to put languages in your Pod. They may find that just languages is a motivation to do it.
...
* TimBL: No. Just the profile code would generate the QR code. There's no statement in the profile. It's just the SolidOS UI generates it.
* SC: We may need to define the property if we want that to be accessible without JavaScript.
* TimBL: Who do we trust to sign VCs? You should just be able to fork data, put it on the web somewhere and sign it. There's a lot of interesting stuff.
