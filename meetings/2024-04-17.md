...
* eP: also starting implementating WebPushChannel2023 (and writing updated draft), plus enabling SAI Authorization Agent to act as Webhooks to Web Push bridge between apps and storages.
...
* SC: I have finished implementing TypeIndex as part of Dokieli. It discovers where to write annotations. It was already detecting other channels (via `solid:storage`, `as:outbox`, `oa:annotationService`); now it also uses TypeIndex. The UI is insteresting, in distinguishing public and private indexes. The spec is simple to implement. It could be useful for a lot of people.
...
* SC: A malicious app could do something with the information it gets from the type index. But they would still not have access to the data itself, only its location.
* PAC: My experience with using Solid application, whenever I use an app, I give it access to my pod. Is the discussion about filtering what the application can access? Is it implemented? Is it possbile today to prevent an app from accessing some part of my pod?
* SC: There is an open questions whether client identifiers are needed. This is one way, but there are alternative approaches. It is hard to get into it right now. There are questions whether client identifiers only limit clients to use OIDC. I think we may need to think more broadly. There are aspects related to signatures, which Social Web is picking up. ActivityPub is using signatures:
...
* eP: this is about streaming HTTP with CSS. Demo uses a public resource but works with access-protected resources as well. Similiar to the old web-socket, but works on standard HTTP, including for authentication.
...
* PAC: I dont remember whether ED says CG draft. I think so.  Will check. 
...
* eP: if you want to send it by the end of this week, we need to sort the name questio by then.
...
* PAC: I understand that the prefernce was to pick one or use placeholder
