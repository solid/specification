...
* eP: STM a week ago, JZ, VB, MDJ present. What's the criteria of accepting a proposal? For me, is to not conflict with the Solid protocol. Nobody showed me any problem with conformance. Discussing counter measures for attacks. My impression is that sometimes we put too much in a PR. If someone things something more needs to be added, they can create a new PR. IMHO this PR is ready to merge. If later we decide we can have more normative requirements, it can be changed.
* JZ: I objec to this PR. We have nothing in place to describe what a webid document does. I agree that we need to take into account security and keep webid docs outside solid servers is a valid option. Recommending that without anything else in place is only going to create a mess. I think it's premature to now start talking about where it leaves without first describing what a webid doc is. There is no consistency.
* eP: I am aware of webid profile draft. Not ignoring that this work exist. Webid profile spec is not completely useless if webid is stored outside Solid. I have this diagram (screen shared). Solid protocol has a normative reference to the webid spec. I think we agreed last week that we need to agree about discovery, make parts public, some protected, but we're not there yet. I am having my WebID ouside the storage. It's a well established practice.
* JZ: I am not trying to prevent people from having the document outside storage. I tried for 6 months to add a predicate for allowing a webid doc to be stored outside Solid. I think the content of webid should be established first. Discovery is important. I don't want to discuss it later. That extends the period of ambiguity for developers.
* eP: there are already at least 2 methods SAI and type indexes. This PR proposes a valid method. I think we have to have a method to separate concerns. I agree it took to long. I recognize the issue, but I don't think it's fair to block this PR. If you have a different option, please make a PR, you can make it option, WIP, and I am happy to add another option. I don't think we can wait adding a counter measure to a known security issue.
* JZ: Sarven and VB have similar objections to mine. If you can convince all of us, ok, but you didn't do that yet.
* eP: maybe vote next week, try to reach consensus. SC and VB do not host webid doc in the solid server.
