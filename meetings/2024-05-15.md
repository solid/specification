...
* eP: Keep in mind we plan to hand over wrk to WG. WG has full mandate to make breaking changes. This is just CG findings. What will we do when breaking changes happen. Few approaches/experiences: Websockets, Solid-OIDC. We need a clear strategy and communicate to implementers. 
...
* eP: See with implementers and deployments - communication loop to be more aware of actual adoption. Harmful for the ecosystem but an argument is the ecosystem is slow so if need to make breaking changes now is the best time. Good to have strategy to be able to make breaking changes later, now is a good time to test drive deprecating, sunsetting, parts of the protocol. 
